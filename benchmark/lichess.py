...
class LichessClient:
    ...
    def _result_to_pgn(self, winner: str | None) -> str:
        if winner == "white":
            return "1-0"
        elif winner == "black":
            return "0-1"
...
class LichessAnalyzer:
    ...
    def _extract_eval(self, eval_data: dict) -> float | None:
        """Extract numeric evaluation from Lichess cloud eval."""
        try:
            if eval_data.get("pvs"):
                pv = eval_data["pvs"][0]
                if "cp" in pv:
...
async def export_game_to_lichess(pgn: str, token: str | None = None) -> dict[str, Any]:
