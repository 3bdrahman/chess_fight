...
from chess_fight.ui.components.chessboard import chessboard as chessboard_component
...
def main():
    ...
    if st.sidebar.button("▶️ Start New Game", type="primary", use_container_width=True):
        ...
        try:
            # Initialize game with provider-agnostic AI
            ...
            async def ui_update(state: GameState):
                ...
                check_square = chess.square_name(king_square) if state.board.is_check() and king_square is not None else None
                chessboard_component(
                    fen=state.board.fen(),
                    orientation="white",
                    last_move=state.moves[-1].move if state.moves else None,
                    check_square=check_square,
                    legal_moves=[m.uci() for m in list(state.board.legal_moves)[:5]] if state.moves else None,
                    key="live_board"
                )
