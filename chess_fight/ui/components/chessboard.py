...
def chessboard(
    ...
    draggable: bool = True,
    animate: bool = True,
    on_move: Callable[[str, str, str, str], bool] | None = None,
    key: str | None = None
) -> dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    with col1:
        ...
        check_sq = chess.square_name(king_square) if board.is_check() and king_square is not None else None
        result = chessboard(
            fen=board.fen(),
            orientation="white",
            last_move="e2e4" if board.move_stack else None,
            check_square=check_sq,
            legal_moves=[m.uci() for m in list(board.legal_moves)[:5]],
            key="demo_board"
        )
